...
async def manager_checks(page,result):
 await open_tab(page,'templates');await page.locator('#openDesktopTemplateEditor').click();await expect(page.locator('#desktopTemplateEditor')).to_be_visible();await expect(page.locator('#dteBody')).to_have_value('متن ذخیره‌شده state1');await page.locator('#dteSubject').fill('موضوع ذخیره آزمایشی');await page.locator('#dteBody').fill('متن ذخیره آزمایشی');await page.locator('#dteSave').click();await expect(page.locator('#desktopTemplateEditor')).not_to_be_visible();await page.locator('#openDesktopTemplateEditor').click();await expect(page.locator('#dteSubject')).to_have_value('موضوع ذخیره آزمایشی');await expect(page.locator('#dteBody')).to_have_value('متن ذخیره آزمایشی');await page.locator('#dteCancel').click();await idle_mutations(page,['#openDesktopTemplateEditor']);result['template_editor_save']='pass';await home(page)
 await open_tab(page,'requestReport');await expect(page.locator('#requestReportView [data-report-export]')).to_have_text('خروجی اکسل');await idle_mutations(page,['#requestReportView [data-report-export]']);await home(page);result['request_report_no_loop']='pass'
 await open_tab(page,'responseReport');rows=page.locator('#responseReportBody tr[data-delivery-id]');await expect(rows).to_have_count(3);delete=page.locator('[data-response-bulk-delete]');await rows.first.click();await expect(delete).to_be_enabled();await delete.click();await page.locator('[data-notice-cancel]').click();await expect(rows).to_have_count(3);await page.evaluate("__testApi.fail.push('cancel_message_deliveries')");await delete.click();await page.locator('[data-notice-ok]').click();await expect(page.locator('#bamcoNoticeDialog')).to_have_attribute('data-kind','error');await page.locator('[data-notice-ok]').click();await page.evaluate('__testApi.fail=[]');await delete.click();await page.locator('[data-notice-ok]').click();await expect(rows).to_have_count(2);await page.locator('[data-notice-ok]').click();await page.wait_for_timeout(400);await rows.nth(0).click();await rows.nth(1).click(modifiers=['Control']);await expect(delete).to_have_text('حذف ۲ رکورد');await delete.click();await page.locator('[data-notice-ok]').click();await expect(rows).to_have_count(0);await page.locator('[data-notice-ok]').click();await home(page);result['response_delete']='pass'
 await open_tab(page,'performanceReport');await expect(page.locator('[data-performance-from]')).to_be_visible();await expect(page.locator('[data-performance-to]')).to_be_visible();assert await page.locator('[data-performance-clear]').evaluate("n=>getComputedStyle(n).fontWeight") in ['400','normal'];await home(page);result['performance_report']='pass'
 await open_tab(page,'messageCenter');assert await page.locator('#messageCenterView th').all_text_contents()==['نام','کار فعال','هشدار','دیرکرد','وضعیت پیام','آخرین ارسال'];await expect(page.locator('#messageChannel')).to_be_visible();assert await page.locator('#messageSubject,#messageCustomText').count()==0;assert await page.locator('#messageCenterView .suite-table-options').count()==0;await page.locator('#messageCenterBody tr[data-id]').first.click();await expect(page.locator('#sendSelectedMessages')).to_be_enabled();await page.locator('#sendSelectedMessages').click();await expect(page.locator('#messagePreviewDialog')).to_be_visible();await expect(page.locator('#messagePreviewDialog .workflow-message')).to_be_visible();await page.locator('[data-message-preview-close]').first.click();await home(page);result['simple_message_send']='pass'
 await open_tab(page,'sentMessages');assert (await page.locator('#sentMessagesView th').all_text_contents())[0]=='فرستنده';await expect(page.locator('#sentLogBody')).to_contain_text('مدیر آزمایشی');assert await page.locator('#sentMessagesView .suite-table-options').count()==0;await home(page);result['sent_log_sender']='pass'
 ...
 await open_tab(page,'kanban');row=page.locator('#kanbanBody tr[data-task-id="1"]');await row.click();history=page.locator('#kanbanView [data-task-history]');await expect(history).to_be_visible();await history.click();await expect(page.locator('#bamcoTaskHistoryDialog')).to_be_visible();txt=await page.locator('#bamcoTaskHistoryDialog').inner_text();assert 'متولی' in txt and 'تاریخ پایان' in txt and 'وضعیت' in txt and 'متولی آزمایشی' in txt;assert 'owner_id' not in txt and 'due_date' not in txt;await page.locator('#bamcoTaskHistoryDialog .bth-close').click();await home(page);result['persian_task_history']='pass'
...
async def case(browser,base,offline,width,role):
 ...
 try:
  ...
  else: assert not await page.locator('#nav [data-view="templates"]').is_visible();result['owner_manager_controls_hidden']='pass'
