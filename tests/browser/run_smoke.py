...
async def manager_checks(page,result):
 ...
 await open_tab(page,'messageCenter');assert await page.locator('#messageCenterView th').all_text_contents()==['نام','کار فعال','هشدار','دیرکرد','وضعیت پیام','آخرین ارسال'];controls=await command_texts(page,'#messageCenterView .message-command-row');assert controls[:4]==['بازگشت به خانه','خروجی اکسل','تازه‌سازی','ارسال'];await expect(page.locator('#messageChannel')).to_be_visible();assert await page.locator('#messageSubject,#messageCustomText').count()==0;assert await page.locator('#messageCenterView .suite-table-options').count()==0;await page.locator('#messageCenterBody tr[data-id]').first.click();send=page.locator('#sendSelectedMessages');await expect(send).to_be_enabled();assert await send.evaluate("n=>getComputedStyle(n).backgroundColor")=='rgb(33, 135, 100)';await send.click();await expect(page.locator('#messagePreviewDialog')).to_be_visible();await expect(page.locator('#messagePreviewDialog .workflow-message')).to_be_visible();await page.locator('[data-message-preview-close]').first.click();await home(page);result['simple_message_send']='pass'
 await open_tab(page,'sentMessages');controls=await command_texts(page,'#sentMessagesView .sent-command-row');assert controls[:3]==['بازگشت به خانه','خروجی اکسل','تازه‌سازی'];headers=await page.locator('#sentMessagesView th').all_text_contents();assert headers==['ردیف','نوع','فرستنده','گیرنده','موضوع','کانال','وضعیت','زمان ارسال','تلاش','خطا','جزئیات'];assert await page.locator('#sentMessagesView .sent-overview,#sentMessagesView .sent-log-summary').count()==0;await expect(page.locator('#sentSearch')).to_be_visible();await expect(page.locator('#sentStatusFilter')).to_be_visible();await expect(page.locator('#sentChannelFilter')).to_be_visible();await expect(page.locator('#sentMessagesView tbody')).to_contain_text('مدیر آزمایشی');await expect(page.locator('#sentMessagesView tbody')).to_contain_text('به‌روزرسانی وظیفه');await expect(page.locator('#sentMessagesView tbody')).to_contain_text('پیام داخل سامانه');assert await page.locator('#sentMessagesView .suite-table-options').count()==0;await home(page);result['sent_log_unified_without_cards']='pass'
 await open_tab(page,'responseTracking');controls=await command_texts(page,'#responseTrackingView .response-command-row');assert controls[0]=='بازگشت به خانه';assert 'از تاریخ' in controls[1] and 'تا تاریخ' in controls[1];assert controls[2]=='تازه‌سازی' and controls[3]=='خروجی اکسل' and controls[4]=='ارسال یادآوری';assert await page.locator('#responseFrom').input_value();assert await page.locator('#responseTo').input_value();assert 'شناسه پیگیری' not in ''.join(await page.locator('#responseTrackingView th').all_text_contents());await home(page);result['response_tracking_root_controls']='pass'
