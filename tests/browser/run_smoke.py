...
async def manager_checks(page,result):
 ...
 await open_tab(page,'sentMessages');headers=await page.locator('#sentMessagesView th').all_text_contents();assert headers[:6]==['شناسه','فرستنده','گیرنده','موضوع','کانال','وضعیت'];await expect(page.locator('#sentMessagesView .sent-overview')).to_be_visible();await expect(page.locator('#sentSearch')).to_be_visible();await expect(page.locator('#sentStatusFilter')).to_be_visible();await expect(page.locator('#sentChannelFilter')).to_be_visible();await expect(page.locator('#sentMessagesView tbody')).to_contain_text('مدیر آزمایشی');assert await page.locator('#sentMessagesView .suite-table-options').count()==0;await home(page);result['sent_log_comprehensive']='pass'
