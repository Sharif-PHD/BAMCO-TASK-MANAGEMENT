...
test('message selection, recipient channels, preview cancel, queue and delivery error are bound',async t=>{
 ...
 const prepared=f.calls.find(c=>c.endpoint==='prepare_workflow_messages');assert.equal(prepared.body.p_channels['test-owner'],'email');assert.equal(prepared.body.p_template_text,'سلام [نام]');assert(d.querySelector('#messagePreviewContent').textContent.includes('متن آزمایشی'));assert(!f.calls.some(c=>c.endpoint==='queue_message_batch'));
