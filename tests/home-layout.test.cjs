...
function fixture(){
 ...
 for(const key of ['conversations','vehicle','tasks','configuration','reports','messages','people']){
...
test('seven cards and each subsection follow the approved RTL reference',()=>{
 const f=fixture();for(const [,ids] of f.order)for(const id of [...ids].reverse())f.button(id);
 f.sync();assert.deepEqual(f.nav.children.map(x=>x.dataset.group),['people','messages','reports','configuration','tasks','vehicle','conversations']);
...
test('home base styles and final post-welcome correction load once in shipped order',()=>{
 ...
 const stable=sheets.findIndex(x=>/home-stable\.css/.test(x)),post=sheets.findIndex(x=>/home-post-welcome-fix-20260911\.css/.test(x));assert.ok(stable>=0);assert.ok(post>stable);assert.equal(post,sheets.length-1);
