...
begin;
do $$
declare manager_id uuid; owner_id uuid; other_id uuid; tid uuid; mid bigint; bid uuid; system_tid uuid; system_mid bigint; reply_id bigint;
begin
 ...
 select id into system_mid from public.chat_messages where thread_id=system_tid and source_delivery_id in(select id from public.message_deliveries where batch_id=bid) and is_system and sender_id is null;
 if system_mid is null then raise exception 'system delivery was not linked'; end if;
 if not exists(select 1 from public.notifications where user_id=owner_id and entity_id=system_tid::text and notification_type='daily') then raise exception 'daily notification missing'; end if;
 perform public.queue_message_batch(bid);
 if (select count(*) from public.chat_messages where source_delivery_id in(select id from public.message_deliveries where batch_id=bid))<>1 then raise exception 'duplicate system message'; end if;
