...
begin;
do $$
declare qa_owner uuid; approver uuid; qa_request_id bigint; result text; current_request public.change_requests%rowtype; rounds integer:=0; denied boolean; payload jsonb; phase int; original_task bigint;
begin
 ...
 rounds:=0;
 payload:=jsonb_build_object('title','__BAMCO_QA_APPROVAL_ROLLBACK__','description','Preserve full request details','status','در حال انجام','owner_id',qa_owner,'priority','متوسط','start_date',current_date,'due_date',current_date+phase*10,'reminder_days',phase*3);
 qa_request_id:=public.submit_change_request(case when phase=1 then 'create' else 'update' end,original_task,payload,'preserve note');
 if (select proposed_data from public.change_requests where id=qa_request_id)<>payload then raise exception 'Submitted payload overwritten'; end if;
 denied:=false;
 ...
 if not exists(select 1 from public.tasks where id=current_request.applied_task_id and owner_id=qa_owner and start_date=current_date and due_date=current_date+phase*10 and description='Preserve full request details' and reminder_days=phase*3) then raise exception 'Approved task lost submitted fields in phase %',phase; end if;
 original_task:=current_request.applied_task_id;
