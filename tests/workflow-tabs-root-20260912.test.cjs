...
function regularButtons(f,selector){for(const button of f.d.querySelectorAll(selector))assert.equal(f.w.getComputedStyle(button).fontWeight,'400')}

test('dashboard performance range auto-applies and exposes only clear action',async t=>{
 const f=await fixture();t.after(()=>f.dispose());await f.open('dashboard');await pause(40);
...
test('response tracking owns one command row, current-month dates, selection and has no tracking id',async t=>{
 ...
 const texts=directTexts(bar);assert.match(texts[0],/بازگشت به خانه/);assert.match(texts[1],/از تاریخ/);assert.match(texts[2],/تازه‌سازی/);assert.match(texts[3],/خروجی اکسل/);assert.match(texts.at(-1),/ارسال یادآوری/);
 assert.equal(f.d.querySelectorAll('#responseTrackingView .response-filters,#responseTrackingView .response-quick').length,0);
 for(const id of ['reminderSendChannel','responsePerson','responseChannel','responseState'])assert.ok(f.d.querySelector('#responseTrackingView .response-command-row #'+id));
 regularButtons(f,'#responseTrackingView .response-command-row button');
...
test('response report has requested command order, month defaults, no search and regular buttons',async t=>{
 ...
 assert.equal(f.d.querySelector('#canonicalResponseSearch'),null);assert.equal(f.d.querySelector('.response-search-row'),null);regularButtons(f,'#responseReportView .response-command-row button');
...
test('message center consolidates controls, removes search, puts channel after refresh and preserves send selection',async t=>{
 ...
 assert.deepEqual(texts.slice(0,3),['بازگشت به خانه','خروجی اکسل','تازه‌سازی']);assert.match(texts[3],/کانال ارسال/);assert.match(texts[4],/ارسال/);
 assert.equal(f.d.querySelector('#messageCenterSearch'),null);assert.equal(f.d.querySelector('#messageCenterView .message-center-simple-toolbar'),null);assert.ok(bar.querySelector('#messageChannel'));regularButtons(f,'#messageCenterView .message-command-row button');
...
test('sent messages uses unified log and consolidates all controls into first row',async t=>{
 ...
 assert.equal(f.d.querySelector('#sentMessagesView .sent-controls'),null);for(const id of ['sentSearch','sentStatusFilter','sentChannelFilter'])assert.ok(bar.querySelector('#'+id));regularButtons(f,'#sentMessagesView .sent-command-row button');
