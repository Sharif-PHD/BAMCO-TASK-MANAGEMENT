...
test('complete emoji groups are selectable without losing drafts; emoji sequences and image stickers keep shared sizing',async t=>{
 ...
 await f.open('groupChat');await until(()=>d.querySelector('.chat-sticker-toggle'));d.querySelector('.chat-sticker-toggle').click();await until(()=>d.querySelector('.chat-emoji-choice'));
 ...
 const input=d.querySelector('.messenger-compose textarea');input.value='سلام ';input.setSelectionRange(input.value.length,input.value.length);const emoji=d.querySelector('.chat-emoji-choice').textContent;d.querySelector('.chat-emoji-choice').click();assert.equal(input.value,'سلام '+emoji);assert.equal(f.messages.length,0);
