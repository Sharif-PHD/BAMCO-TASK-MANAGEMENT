...
const note='        <div class="feature-confidential-note">رمز عبور فقط با درخواست صریح شما از سرور امن دریافت می‌شود و به‌صورت پیش‌فرض نمایش داده نمی‌شود.</div>\n';
if(!html.includes(note))throw new Error('password note not found');
html=html.replace(note,'').replace('assets/js/documents-sites.js?v=documents-sites-20260912-1','assets/js/documents-sites.js?v=documents-sites-20260912-2').replace('assets/js/card-home.js?v=welcome-home-20260911-2','assets/js/card-home.js?v=welcome-home-20260912-3');
...
const checks={
  ...
  oldModal:!preview.includes('documentPreviewDialog'),
  noteRemoved:!html.includes('رمز عبور فقط با درخواست صریح شما'),
