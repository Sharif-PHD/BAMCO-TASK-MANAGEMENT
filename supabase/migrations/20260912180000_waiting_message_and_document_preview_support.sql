-- Canonical support for waiting-response work in automatic messages and robust document MIME handling.
alter table public.message_snapshots
  add column if not exists waiting_count integer not null default 0,
  add column if not exists waiting_task_ids bigint[] not null default '{}'::bigint[];

update public.message_snapshots s
set waiting_task_ids = coalesce((
      select array_agg((x->>'id')::bigint order by (x->>'id')::bigint)
      from jsonb_array_elements(coalesce(s.tasks,'[]'::jsonb)) x
      where coalesce(x->>'status_key','')='waiting' or btrim(coalesce(x->>'status',''))='منتظر پاسخ'
    ),'{}'::bigint[]),
    waiting_count = coalesce((
      select count(*)
      from jsonb_array_elements(coalesce(s.tasks,'[]'::jsonb)) x
      where coalesce(x->>'status_key','')='waiting' or btrim(coalesce(x->>'status',''))='منتظر پاسخ'
    ),0)
where coalesce(s.waiting_count,0)=0 or coalesce(cardinality(s.waiting_task_ids),0)=0;

update storage.buckets
set allowed_mime_types = (
  select array_agg(distinct mime order by mime)
  from unnest(coalesce(allowed_mime_types,'{}'::text[]) || array['image/gif']) mime
)
...
begin
  ...
  for rid in select distinct unnest(p_recipient_ids) loop
    ...
    select count(*),
           count(*) filter(where due_state='warning'),
           count(*) filter(where due_state='overdue'),
           count(*) filter(where status_kind='waiting'),
           coalesce(array_agg(id order by id),'{}'),
           coalesce(array_agg(id order by id) filter(where due_state='warning'),'{}'),
    ...
    v_final:=replace(replace(replace(replace(
      ...
      '[استیکر]','');
    if position('[جدول امور منتظر پاسخ]' in v_body)=0 then
      v_final:=v_final||E'\n\nامور منتظر پاسخ:\n'||coalesce(waiting_text,'موردی در انتظار پاسخ نیست.');
    end if;
