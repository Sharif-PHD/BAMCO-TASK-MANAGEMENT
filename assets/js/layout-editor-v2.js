(() => {
  ...
  function resetAll(){if(!confirm('همه جابه‌جایی‌های دستی بازنشانی شود؟'))return;Object.entries(state.layouts).forEach(([s,rec])=>{try{$$(s).forEach(el=>Object.keys(rec.style||{}).forEach(p=>el.style.removeProperty(p)))}catch{}});state.layouts={};persist();select(null)}
  ...
  async function importLayout(e){const f=e.target.files?.[0];if(!f)return;try{const o=JSON.parse(await f.text());state.layouts=o.layouts||o;persist();applyAll();select(null)}catch{alert('فایل چیدمان معتبر نیست.')}finally{e.target.value=''}}
