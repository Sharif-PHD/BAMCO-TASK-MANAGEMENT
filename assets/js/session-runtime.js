...
const KEY='bamco.session.id.v1';let starting=null,confirmedId=null,ending=false,generation=0,lastActivity=Date.now(),lastSent=0;
...
function start(){
 ...
 if(!starting){
  ...
  const request=call('start',{app_version:'sessions-20260910-4'}).then(data=>{
   const id=data?.session?.id;if(!id)throw Error('ثبت نشست انجام نشد؛ دوباره وارد شوید.');
   if(epoch===generation&&!ending&&state.token){sessionStorage.setItem(KEY,id);confirmedId=id;lastActivity=Date.now();lastSent=lastActivity}
   return id;
