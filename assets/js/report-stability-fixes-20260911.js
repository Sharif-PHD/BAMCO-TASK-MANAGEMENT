...
function installCss(){
 ...
 #responseReportView [data-response-bulk-delete]{color:#b54040!important;border-color:#e3aaaa!important;background:#fff5f5!important;white-space:nowrap!important;font-weight:400!important}
 ...
 @media(max-width:900px){#responseReportView .response-command-row .canonical-date-controls{width:100%}}
...
async function renderResponse(force=false){
 ...
 try{responseRows=await selectAll('message_response_tracking','select=*&order=sent_at.desc');window.__bamcoResponseRows=responseRows}catch(err){if(epoch===renderEpoch&&state.view==='responseReport')showError(view,err,'responseReport');return}
 if(epoch!==renderEpoch||state.view!=='responseReport')return;renderResponseRows();
}
...
async function syncCore(){
 ...
 try{const settled=await Promise.allSettled([selectAll('task_status_view','select=*&order=id.desc'),window.bamcoRequestSync?.refresh?.(),window.bamcoInbox?.load?.(),window.bamcoConversations?.refresh?.()]);if(userId!==state.user?.id||token!==state.token)return;const tasks=settled[0];if(tasks.status==='fulfilled'){state.tasks=tasks.value;if(state.view==='kanban')renderTasks(false);else if(state.view==='archive')renderTasks(true);else if(state.view==='dashboard')window.renderDashboard?.()}if(state.view==='performanceReport')await renderPerformance(false);else if(state.view==='responseReport')await renderResponse(false);else if(state.view==='sentMessages')q('#sentMessagesView [data-sent-refresh]')?.click();document.dispatchEvent(new CustomEvent('bamco-live-sync',{detail:{at:Date.now()}}))}catch(err){console.warn('15-second live sync',err?.message||err)}finally{syncBusy=false}
}
