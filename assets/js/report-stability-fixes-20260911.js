...
async function syncCore(){
 if(syncBusy||typeof state==='undefined'||!state?.token||!state?.profile||state.profile.must_change_password||document.hidden)return;syncBusy=true;const userId=state.user?.id,token=state.token;
