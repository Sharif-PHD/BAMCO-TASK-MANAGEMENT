...
async function syncCore(){
 if(syncBusy||typeof state==='undefined'||!state?.token||!state?.profile||state.profile.must_change_password||document.hidden)return;syncBusy=true;const userId=state.user?.id,token=state.token,taskRevision=state.taskRevision||0;
 try{const settled=await Promise.allSettled([selectAll('task_status_view','select=*&order=id.desc'),window.bamcoRequestSync?.refresh?.(),window.bamcoInbox?.load?.(),window.bamcoConversations?.refresh?.()]);if(userId!==state.user?.id||token!==state.token)return;const tasks=settled[0];if(tasks.status==='fulfilled'&&taskRevision===(state.taskRevision||0)){state.tasks=tasks.value;if(state.view==='kanban')renderTasks(false);else if(state.view==='archive')renderTasks(true);else if(state.view==='dashboard')window.renderDashboard?.()}if(state.view==='performanceReport')await renderPerformance(false);else if(state.view==='responseReport')await renderResponse(false);else if(state.view==='sentMessages')q('#sentMessagesView [data-sent-refresh]')?.click();document.dispatchEvent(new CustomEvent('bamco-live-sync',{detail:{at:Date.now()}}))}catch(err){console.warn('15-second live sync',err?.message||err)}finally{syncBusy=false}
}
