...
function boot(){installCss();watch();purgeSystemThreads();new MutationObserver(purgeSystemThreads).observe(document.body,{childList:true,subtree:true});document.addEventListener('click',e=>{const nav=e.target.closest('#nav [data-view]');if(nav&&reportIds.has(nav.dataset.view))begin(nav.dataset.view);const refresh=e.target.closest('[data-tab-refresh]');if(refresh&&reportIds.has(refresh.dataset.tabRefresh))begin(refresh.dataset.tabRefresh)},true)}
