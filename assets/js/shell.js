...
(()=>{
  ...
  function syncMessageAvailability(){const f=q('#personForm'),hasEmail=!!f.elements.email.value.trim(),channel=f.elements.default_message_channel;channel.disabled=false;[...channel.options].forEach(o=>o.disabled=o.value!=='portal'&&!hasEmail);if(!hasEmail||!channel.value)channel.value='portal';q('#personLoginActions').classList.toggle('hidden',hasEmail||!editing)}
...
(()=>{
 const q=s=>document.querySelector(s),nav=q('#nav'),workspace=q('.workspace');if(!nav||!workspace)return;let rows=[],alerts=[],epoch=0,lastAlertSignature='',threadNames=new Map();const kinds={public_chat:'گفت‌وگوی عمومی',group_chat:'گروه',direct_chat:'شخصی',task_chat:'گفت‌وگوی وظیفه',daily:'گزارش وضعیت امور روزانه',reminder:'یادآور',manual:'پیام سامانه',system:'پیام سامانه',system_reply:'پاسخ به پیام سامانه'};
