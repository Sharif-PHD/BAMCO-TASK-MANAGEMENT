...
(()=>{
  ...
  function syncPersonSelection(){
    ...
    q('#editPersonBtn').disabled=!valid;q('#deletePersonBtn').disabled=!valid;
  }
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    q('#nav').addEventListener('click',e=>{const view=e.target.closest('button[data-view]')?.dataset.view;if(view==='people'&&isManager())loadPeople().catch(err=>toast(err.message,true));if(view==='templates'&&isManager())setTimeout(loadTemplates);if(view==='settings')setTimeout(loadSettings)});
  });
})();
