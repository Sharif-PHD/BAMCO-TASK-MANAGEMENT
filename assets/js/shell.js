...
(()=>{
  ...
  async function savePerson(e){e.preventDefault();const data=Object.fromEntries(new FormData(e.currentTarget));data.active=data.active==='true';data.email=String(data.email||'').trim()||null;data.messaging_enabled=!!data.email;data.default_message_channel=data.email?(data.default_message_channel||'portal'):'none';if(editing)data.user_id=editing.id;try{await edge(data);q('#personDialog').close();toast('اطلاعات فرد ذخیره شد.');await loadPeople()}catch(err){toast(err.message,true)}}
