...
(()=>{
  ...
  let people=[],editing=null,selected=null;
  function syncPersonSelection(){
    const valid=people.some(p=>p.id===selected);if(!valid)selected=null;
    document.querySelectorAll('#peopleBody tr[data-id]').forEach(row=>{const chosen=row.dataset.id===selected;row.classList.toggle('person-selected',chosen);row.setAttribute('aria-selected',String(chosen))});
    q('#editPersonBtn').disabled=!people.length;q('#editPersonBtn').title=valid?'ویرایش فرد انتخاب‌شده':'انتخاب فرد برای ویرایش';q('#deletePersonBtn').disabled=!valid;
  }
  ...
  function pickPersonToEdit(){
    const person=people.find(p=>p.id===selected);if(person)return openPerson(person);
    ...
    q('button[type=button]',dialog).onclick=()=>dialog.close();q('form',dialog).onsubmit=e=>{e.preventDefault();const p=people.find(p=>p.id===e.currentTarget.elements.person.value);if(!p)return;selected=p.id;syncPersonSelection();dialog.close();openPerson(p)};dialog.showModal();
  }
...
(()=>{
  ...
  async function loadPeople(){const people=await select('profiles','active=eq.true&messaging_enabled=eq.true&select=id,full_name,email&order=full_name');document.querySelector('#messageRecipients').innerHTML=people.filter(p=>p.id!==state.profile.id).map(p=>{const tasks=state.tasks.filter(t=>!t.archived&&t.owner_id===p.id),warning=tasks.filter(t=>norm(t.due_state)==='دوره هشدار').length,late=tasks.filter(t=>norm(t.due_state)==='دیرکرد').length;return`<option value="${p.id}">${esc(p.full_name)} — هشدار ${fa(warning)}، دیرکرد ${fa(late)}</option>`}).join('');return people}
  document.addEventListener('DOMContentLoaded',()=>{
