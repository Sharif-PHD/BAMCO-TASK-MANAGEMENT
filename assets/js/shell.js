...
(()=>{
  ...
  function showInitialCredentials(result){let dialog=q('#initialCredentials');if(!dialog){dialog=document.createElement('dialog');dialog.id='initialCredentials';dialog.className='modal bamco-dialog';document.body.append(dialog)}dialog.innerHTML=`<div class="modal-head"><h3>حساب ساخته شد</h3></div><p>رمز موقت فقط همین‌بار نمایش داده می‌شود. آن را امن به کاربر تحویل دهید؛ کاربر در اولین ورود رمز خود را تغییر می‌دهد.</p><label>نام کاربری<input class="english" dir="ltr" readonly value="${esc(result.login_name)}"></label><label>رمز موقت<input class="english" dir="ltr" readonly value="${esc(result.temporary_password)}"></label><div class="modal-actions"><button type="button" class="primary">ثبت کردم؛ بستن</button></div>`;q('button',dialog).onclick=()=>dialog.close();dialog.addEventListener('close',()=>dialog.replaceChildren(),{once:true});dialog.showModal()}
