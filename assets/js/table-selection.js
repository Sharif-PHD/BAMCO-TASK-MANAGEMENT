...
const taskScope=t=>t?.closest('#kanbanView')?'kanban':t?.closest('#archiveView')?'archive':null;
...
function clear(value){const t=tableOf(value);if(t){const scope=taskScope(t);if(scope&&window.bamcoTaskSelection?.clear){window.bamcoTaskSelection.clear(scope);return}model(t).ids.clear();notify(t)}else for(const m of models.values()){m.ids.clear();if(m.table.isConnected)notify(m.table)}}
...
window.bamcoSelection={ids:value=>{const t=tableOf(value),scope=taskScope(t);return scope&&window.bamcoTaskSelection?.ids?[...window.bamcoTaskSelection.ids(scope)]:t?[...model(t).ids]:[]},rows:value=>{const t=tableOf(value),scope=taskScope(t);if(scope&&window.bamcoTaskSelection?.ids){const selected=new Set(window.bamcoTaskSelection.ids(scope).map(String));return rows(t).filter(r=>selected.has(key(r)))}return t?rows(t).filter(r=>model(t).ids.has(key(r))):[]},clear,set(value,ids){const t=tableOf(value),scope=taskScope(t);if(scope&&window.bamcoTaskSelection?.clear&&window.bamcoTaskSelection?.toggle){window.bamcoTaskSelection.clear(scope);for(const id of ids.map(String))window.bamcoTaskSelection.toggle(scope,id,false);return}if(t){model(t).ids=new Set(ids.map(String));notify(t)}},key};
