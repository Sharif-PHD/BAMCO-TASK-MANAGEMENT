...
async function mount(host,{id,title,subtitle=''}){
 ...
 active=stateUI;stateUI.close=()=>{stateUI.closed=true;stateUI.urls.forEach(URL.revokeObjectURL);stateUI.urls=[]};
 host.classList.add('messenger-host');host.innerHTML=`<div class="messenger-head">${avatar}<div><strong>${esc(title)}</strong><small>${esc(subtitle||'گفتگو')}</small></div><button type="button" class="chat-refresh" aria-label="تازه‌سازی پیام‌ها">↻</button></div><div class="messenger-messages" role="log" aria-live="polite"><div class="chat-empty">در حال دریافت پیام‌ها…</div></div><div class="chat-error" role="status"></div><div class="chat-reply hidden"><span></span><button type="button" aria-label="لغو پاسخ">×</button></div><div class="chat-pending hidden"><span></span><button type="button" aria-label="حذف پیوست">×</button></div><div class="chat-stickers hidden" aria-label="انتخاب استیکر"></div><form class="messenger-compose"><button type="button" class="chat-attach" title="عکس یا فایل تا ۵ مگابایت" aria-label="پیوست عکس یا فایل">＋</button><button type="button" class="chat-sticker-toggle" title="استیکر" aria-label="انتخاب استیکر">☺</button><textarea rows="1" aria-label="متن پیام" placeholder="پیام بنویسید…"></textarea><button type="submit" class="chat-send" aria-label="ارسال پیام">➤</button><input type="file" hidden></form>`;
 ...
 async function storage(path,options={}){const res=await fetch(SB_URL+'/storage/v1/object/'+path,{...options,headers:{apikey:SB_KEY,Authorization:'Bearer '+state.token,...options.headers}});if(!res.ok)throw new Error('دسترسی به پیوست انجام نشد. مخزن خصوصی فایل و دسترسی‌های آن باید در سرور فعال باشد.');return res}
 ...
 async function send(sticker){if(stateUI.busy||stateUI.closed)return;let text=input.value.trim(),uploaded=null;if(!sticker&&!text&&!stateUI.pending)return;stateUI.busy=true;q('.chat-send').disabled=true;error('');
  try{if(sticker)text=STICKER+sticker;
   else if(stateUI.pending){const file=stateUI.pending;if(!fileAllowed(file))throw Error('فایل باید حداکثر ۵ مگابایت باشد.');const name=file.name.replace(/[^\w.\-\u0600-\u06ff]/g,'_'),path=id+'/'+state.user.id+'/'+crypto.randomUUID()+'/'+name;await storage('chat-attachments/'+path.split('/').map(encodeURIComponent).join('/'),{method:'POST',headers:{'Content-Type':file.type||'application/octet-stream','x-upsert':'false'},body:file});uploaded=path;text=FILE+JSON.stringify({path,name:file.name,size:file.size,mime:file.type,caption:text})}
 ...
 await load();
 async function tick(){if(stateUI.closed||!host.isConnected)return;if(!document.hidden&&!host.closest('.view')?.classList.contains('hidden'))await load();if(!stateUI.closed&&host.isConnected)setTimeout(tick,5000)}setTimeout(tick,5000);
}
window.bamcoChat={mount};
})();
