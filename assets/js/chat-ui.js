...
async function mount(host,{id,title,subtitle='',actions=[],personId=null,groupPhoto='',readOnly=false}){
 if(active)active.close();
 if(!host)throw Error('پنل گفتگو آماده نیست.');
 const stateUI={id,reply:null,editing:null,pending:null,files:new Map(),busy:false,closed:false,urls:[],messages:[],loaded:false};
 ...
 async function load(manual=false){
  if(stateUI.closed)return;try{const [messages,people]=await Promise.all([selectAll('chat_messages',`select=*&thread_id=eq.${encodeURIComponent(id)}&deleted_at=is.null&order=created_at.asc`,200),directory()]);if(stateUI.closed||!host.isConnected)return;
   ...
   if(!stateUI.loaded||nearBottom||manual)box.scrollTop=box.scrollHeight;else box.scrollTop=oldTop;stateUI.loaded=true;await rpc('chat_mark_read',{p_thread_id:id});document.dispatchEvent(new CustomEvent('bamco-messages-changed'));
 ...
 q('.chat-sticker-toggle').onclick=async()=>{
  ...
  try{const groups=await loadEmoji();if(stateUI.closed)return;
   ...
   const paint=()=>{grid.replaceChildren();const term=search.value.trim().toLowerCase();
    ...
    items.slice(0,limit).forEach(([glyph,name])=>{const b=document.createElement('button');b.type='button';b.className='chat-emoji-choice';b.textContent=glyph;b.title=name;b.setAttribute('aria-label','انتخاب '+name);b.onclick=()=>{input.setRangeText(glyph,input.selectionStart??input.value.length,input.selectionEnd??input.value.length,'end');input.dispatchEvent(new Event('input',{bubbles:true}));panel.classList.add('hidden');input.focus()};grid.append(b)});
