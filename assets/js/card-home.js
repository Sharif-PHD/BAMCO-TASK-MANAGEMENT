...
function install(){
 ...
 function openWaitingKanban(){const tasks=typeof state!=='undefined'&&Array.isArray(state.tasks)?state.tasks:[],sample=tasks.find(t=>!t.archived&&window.bamcoOptions?.kind?.(t)==='waiting'),label=window.bamcoOptions?.label?.('status','waiting')||sample?.status||'منتظر پاسخ';if(typeof tableFilters!=='undefined'&&tableFilters?.kanban){for(const key of Object.keys(tableFilters.kanban))delete tableFilters.kanban[key];tableFilters.kanban[4]=label}const search=q('#kanbanSearch');if(search)search.value='';leaveHome();if(dialog.open)dialog.close();if(typeof showView==='function')showView('kanban');else q('#nav button[data-view="kanban"]')?.click();requestAnimationFrame(()=>{if(typeof renderTasks==='function')renderTasks(false)})}
