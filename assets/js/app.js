...
(function(){
  ...
  function sizeCanvas(canvas){const dpr=window.devicePixelRatio||1,w=Math.max(window.matchMedia?.("(max-width:760px)").matches?640:320,canvas.parentElement.clientWidth);if(!canvas.dataset.logicalHeight)canvas.dataset.logicalHeight=canvas.getAttribute('height')||'300';const h=Number(canvas.dataset.logicalHeight);canvas.style.setProperty("--chart-width",`${w}px`);canvas.parentElement.tabIndex=0;canvas.parentElement.setAttribute("aria-label","نمودار؛ برای مشاهده کامل افقی پیمایش کنید");canvas.style.width=`${w}px`;canvas.style.height=`${h}px`;if(canvas.width!==Math.round(w*dpr)||canvas.height!==Math.round(h*dpr)){canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr)}const ctx=canvas.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);return{ctx,w,h}}
