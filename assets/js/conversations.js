...
(()=>{
 ...
 window.bamcoConversations={refresh:refreshCurrent,async open(threadId){const rows=await rpc('chat_conversation_list',{}),t=rows.find(x=>x.id===threadId);if(!t)throw Error('این گفت‌وگو در دسترس نیست.');const route=t.task_id?'taskChats':['public','group'].includes(t.thread_type)?'groupChat':'directMessages';showView(route);await render(route);const view=q('#'+route+'View'),button=qa('[data-thread]',view).find(b=>b.dataset.thread===threadId);if(button)await openThread(button,view)},owns:id=>Object.hasOwn(names,id),render,close(){serial++;selectedTask=null;window.bamcoChat?.close()}};
})();
