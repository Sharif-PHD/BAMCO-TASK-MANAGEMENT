...
function ensureButton(id,label,group,managerOnly){let b=q(`#nav button[data-view="${id}"]`);if(!b){b=document.createElement('button');b.type='button';b.dataset.view=id;b.innerHTML=`<b>•</b><span>${esc(label)}</span>`;}if(!b.dataset.runtimeBound){b.dataset.runtimeBound='1';b.addEventListener('click',()=>{setTimeout(()=>render(id),0)})}if(managerOnly)b.classList.add('manager-only');b.classList.toggle('hidden',managerOnly&&!mgr());const g=groupFor(group),box=q('.nav-group-items',g);if(box&&b.parentElement!==box)box.appendChild(b);return b}
...
function bind(){document.addEventListener('click',e=>{const retry=e.target.closest('[data-runtime-retry]');if(retry)render(retry.dataset.runtimeRetry)})}
function install(){if(installed)return;installed=true;NEW.forEach(([id,label,group,m])=>{ensureView(id,m);ensureButton(id,label,group,m)});syncOrder();bind()}
