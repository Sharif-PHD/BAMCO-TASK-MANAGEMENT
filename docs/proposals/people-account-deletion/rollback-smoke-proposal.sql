-- DRAFT / NOT EXECUTED. These assertions have not passed against any database.
...
begin
 begin
  ...
  if not exists(select 1 from public.change_requests where id=reqid and request_status='cancelled' and requested_by is null) then raise exception 'Deleted-user request not closed';end if;
  ...
  if not exists(select 1 from public.chat_messages where thread_id=gid and sender_id is null) then raise exception 'Conversation history lost';end if;
  ...
  for r in select c.oid::regclass tab from pg_class c join pg_namespace ns on ns.oid=c.relnamespace where ns.nspname='public' and c.relkind='r' and c.relrowsecurity loop
   execute format('select count(*) from %s',r.tab) into n;if n<>0 then raise exception 'Deleted identity can still read %',r.tab;end if;
  end loop;
